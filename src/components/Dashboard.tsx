...
import { buildHouseTooltip } from "../lib/astro-data/houseInterpretations";
...
export function Dashboard({
  ...
  return (
    ...
    >
      ...
      {/* ═══ WESTERN ASTROLOGY — 3-COLUMN ROW ═══════════════════════ */}
      <motion.div className="mb-12" {...fadeIn(0.2)}>
        ...
        {/* Sun · Moon · Ascendant — equal-height 3-column grid */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 items-stretch">

          {/* Sun Sign */}
          <div className="morning-card p-5 sm:p-6 flex flex-col justify-between h-full" data-special="true">
            <div>
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="min-w-0">
                  <h3 className="font-serif text-xl text-[#1E2A3A] leading-tight mb-0.5">
                    {sunSignName || "—"}
                  </h3>
                ...
                {(() => { const art = getZodiacArt(sunSign); return art ? (
                  <img src={art} alt={sunSignName} className="w-20 h-20 object-contain shrink-0 -mt-1" loading="lazy" />
                ) : null; })()}
              </div>
              <p className="text-xs text-[#1E2A3A]/55 leading-relaxed">
                {sunSignData ? sunSignData.sun[lang] : t("dashboard.western.sunDesc")}
              </p>
          ...
          {/* Moon Sign */}
          <div className="morning-card p-5 sm:p-6 flex flex-col justify-between h-full">
            <div>
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                {(() => { const art = getZodiacArt(moonSign); return art ? (
                  <img src={art} alt={moonSignName} className="w-20 h-20 object-contain shrink-0 -mt-1" loading="lazy" />
                ) : null; })()}
              </div>
              <p className="text-xs text-[#1E2A3A]/55 leading-relaxed">
                {moonSignData ? moonSignData.moon[lang] : t("dashboard.western.moonDesc")}
              </p>
            ...
            <div>
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                {(() => { const art = getZodiacArt(ascendantSign); return art ? (
                  <img src={art} alt={ascSignName} className="w-20 h-20 object-contain shrink-0 -mt-1" loading="lazy" />
                ) : null; })()}
              </div>
              <p className="text-xs text-[#1E2A3A]/55 leading-relaxed">
                {ascSignData ? ascSignData.asc[lang] : t("dashboard.western.ascDesc")}
              </p>
            </div>
            <div className="flex justify-between items-center border-t border-[#8B6914]/10 pt-4 mt-5">
              <span className="text-2xl leading-none select-none text-[#3D8B37]">{ascEmoji}</span>
      ...
      <PremiumGate teaser={t("dashboard.premium.teaserPillars")}>
        <motion.div className="mb-12" {...fadeIn(0.3)}>
          ...
          {/* Year Animal · Dominant WuXing · Day Master · Month Stem — 4-col row */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">

            {/* Year Animal */}
            <div className="morning-card p-4 sm:p-5 flex flex-col justify-between h-full" data-special="true">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-3xl font-serif leading-none select-none" style={{ color: yearBranch ? '#8B6914' : undefined }}>{yearBranch?.chinese || "✨"}</span>
                  <Badge text={t("dashboard.bazi.zodiacLabel")} />
                </div>
                <h3 className="font-serif text-base text-[#1E2A3A] leading-tight mb-0.5">
                  {yearAnimalName || "—"}
                </h3>
                <p className="text-[9px] uppercase tracking-[0.2em] text-[#8B6914]/50 mb-3">
                  {yearElement && yearBranch
                    ? `${getWuxingName(yearElement, lang)}-${yearAnimalName}`
                    : t("dashboard.bazi.yearAnimalTitle")}
                </p>
                {yearCoinSrc && (
                  <div className="flex justify-center my-2">
                    <img src={yearCoinSrc} alt={yearAnimalName} className="w-20 h-20 object-contain rounded-full" loading="lazy" />
                  </div>
                )}
                {yearBranch && (
                  <p className="text-[10px] text-[#1E2A3A]/55 leading-relaxed line-clamp-3">
                    {yearBranch.description[lang]}
                  </p>
                )}
                <ExpandableText text={tileTexts?.yearAnimal} />
              </div>
              <div className="flex justify-between items-center border-t border-[#8B6914]/10 pt-3 mt-4">
                {yearBranch && (
                  <span className="text-[10px] text-[#1E2A3A]/35">{yearBranch.pinyin}</span>
                )}
            ...
            <div
              className="morning-card p-4 sm:p-5 flex flex-col justify-between h-full"
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-3xl font-serif leading-none select-none" style={{ color: dominantWuxing?.color }}>{dominantWuxing?.chinese || "✨"}</span>
                ...
                {dominantWuxing && (
                  <p className="text-[10px] text-[#1E2A3A]/55 leading-relaxed line-clamp-4">
              ...
              <div className="flex justify-between items-center border-t border-[#8B6914]/10 pt-3 mt-4">
                {dominantWuxing && (
                  <span className="text-[10px] text-[#1E2A3A]/35">
                    {dominantWuxing.pinyin} · {dominantWuxing.direction[lang]}
                  </span>
                )}
                <Badge text="WUXING" />
              </div>
            ...
            {/* Day Master */}
            <div className="morning-card p-4 sm:p-5 flex flex-col justify-between h-full">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-3xl font-serif leading-none select-none text-[#D4AF37]">{dayMaster}</span>
              ...
              <div className="flex justify-between items-center border-t border-[#8B6914]/10 pt-3 mt-4">
                {dayMasterStem && (
                  <span className="text-[10px] text-[#1E2A3A]/35">
                    {dayMasterStem.element} · {dayMasterStem.yinYang === "yang" ? "Yang" : "Yin"}
                  </span>
                )}
            ...
            <div className="morning-card p-4 sm:p-5 flex flex-col justify-between h-full">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-2xl leading-none select-none text-[#8B6914]">月</span>
              ...
              <div className="flex justify-between items-center border-t border-[#8B6914]/10 pt-3 mt-4">
                {monthStemData && (
                  <span className="text-[10px] text-[#1E2A3A]/35">
                    {monthStemData.element} · {monthStemData.yinYang === "yang" ? "Yang" : "Yin"}
                  </span>
                )}
          ...
          {/* Compact WuXing Bar Chart */}
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <p className="text-[9px] uppercase tracking-[0.3em] text-[#8B6914]/50">WuXing 五行</p>
              <Link
                to="/wu-xing"
            ...
            <div className="morning-card px-5 py-4">
                    ...
                    <Tooltip key={el.key} content={el.description[lang]} wide dark={planetariumMode}>
                      <div className="flex items-center gap-3 cursor-help group">
                        <div className="w-20 shrink-0 flex items-center gap-2">
                          <span className="text-lg font-serif leading-none select-none" style={{ color: el.color }}>
                            {el.chinese}
                          </span>
                          <div className="min-w-0">
                            <div className="text-[10px] font-medium text-[#1E2A3A] truncate">{el.name[lang]}</div>
                          </div>
                        </div>
                        <div className="flex-1 h-1.5 rounded-full bg-[#8B6914]/08 overflow-hidden">
                          {hasWuxingData ? (
                            <div
                        ...
                        <div className="w-10 shrink-0 text-right flex items-center justify-end gap-1">
                          ...
                          {isDom && <span className="text-xs" style={{ color: el.color }}>★</span>}
                        </div>
                      </div>
                    </Tooltip>
                  );
      ...
      {/* ═══ WESTERN HOUSES (FR-07: rich personalised tooltips) ═══════ */}
          ...
          <motion.div className="mb-10" {...fadeIn(0.4)}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {houseEntries.map(([houseKey, val]) => {
                ...
                // Rich personalised tooltip: buildHouseTooltip first, AI houseTexts as supplement
                const richTooltip = num !== null && sign
                  ? buildHouseTooltip(num, sign, lang)
                  : (num !== null ? houseTexts?.[String(num)] : undefined);
                ...
                const cardContent = (
                  <>
                    ...
                    <div className="font-serif text-base sm:text-lg text-[#1E2A3A] flex items-center gap-1.5 mb-2 min-w-0">
                    ...
                    {meaning && sign && (
                      <p className="text-[9px] sm:text-[10px] text-[#1E2A3A]/40 leading-relaxed line-clamp-2">
                        {lang === "de"
                          ? `${signDisplay} prägt ${meaning.keyword.de}.`
                          : `${signDisplay} in your house of ${meaning.name.en}.`}
                      </p>
                    )}
                  </>
                );
                ...
                return richTooltip ? (
                  <Tooltip key={houseKey} content={richTooltip} wide dark={planetariumMode}>
                    <div className="morning-card p-4 sm:p-5 overflow-hidden cursor-help h-full">
                      {cardContent}
                    </div>
                  </Tooltip>
                ) : (
                  <div key={houseKey} className="morning-card p-4 sm:p-5 overflow-hidden h-full">
                    {cardContent}
                  </div>
                );
              })}
      ...
      <motion.div className="mb-8" {...fadeIn(0.45)}>
      ...
      {/* ═══ LEVI — CENTERED RECTANGLE CTA ══════════════════════════ */}
      <motion.div className="mb-12 flex justify-center" {...fadeIn(0.5)}>
        {/* backdrop-filter: none prevents stacking-context clipping of the ElevenLabs popup */}
        <div
          ref={leviSectionRef}
          className="morning-card p-6 sm:p-8 w-full max-w-2xl"
          style={{ backdropFilter: 'none', WebkitBackdropFilter: 'none', overflow: 'visible' }}
        >
          {/* Header row */}
          <div className="flex items-start gap-4 mb-6">
            <div className="relative mt-1.5 shrink-0">
              <div className={`w-2.5 h-2.5 rounded-full breathing ${
                leviActive
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] uppercase tracking-[0.35em] text-[#8B6914] mb-1.5 font-semibold">
          ...
          {isPremium ? (
            <>
              <button
                ...
                className={`w-full flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-full text-[10px] uppercase tracking-[0.25em] font-semibold transition-all ${
                  leviActive
              ...
              <AnimatePresence>
                {leviActive && (
                  <motion.div
                    ...
                    className="relative z-[9999] w-full flex justify-center mt-5"
                  >
                    {/* @ts-ignore */}
                    <elevenlabs-convai
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-full text-[10px] uppercase tracking-[0.25em] font-semibold bg-gold/10 border border-gold/30 text-gold hover:bg-gold/20 transition-all disabled:opacity-60 disabled:cursor-wait"
            >
      ...
      {/* ═══ SHARE CARD ═══════════════════════════════════════════════ */}
      <motion.div className="mb-16" {...fadeIn(0.55)}>
        <ShareCard
